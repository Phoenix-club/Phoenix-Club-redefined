...
const Section = ({scrollDiv , setScrollDiv,textContent, setTextContent }) => {
    ...
    const [cursor,  setCursor] = useState('/space/AIM2.png')
    const data = {
        Presidents: {
            ...
            { !isMobile && <MouseFollower hov={hov} cursor={cursor}/>}
                ...
                <div className='h-full max-sm:h-1/2 w-full max-sm:w-full flex flex-col justify-between items-start text-[#fff] max-sm:flex-shrink-0 '>
                    <div className='w-full h-full max-sm:flex'>
                        <div
                        className={` ${ isMobile && `${click ? `translate-x-0 `: `-translate-x-[80%]`} bg-[url("/space/INFO2.png")] overflow-visible bg-cover bg-bottom inset-0 opacity-80 bg-no-repeat` } flex flex-col h-[40vh] items-start max-md:gap-4 gap-7 font-pixelSans text-xl z-10 p-10 w-full transition-all duration-500 ease-in-out`}
                    ...
                    {/* <img src="../src/assets/space/DECOR.png" className='h-10 opacity-60' alt="" /> */}
                            <button 
                                ...
                                onMouseEnter={()=> {
                                    setCursor('/space/SINFO.png')
                                    setHov('Presidents')
                                }} 
                                onMouseLeave={()=>setCursor('/space/AIM2.png')}
                    ...
                    { !isMobile && <div className='h-32 w-full flex opacity-80 justify-start relative transition-all'>
                        ...
                        <img src="/space/DECOR1.png" className='h-32 absolute' alt="" />
                ...
                {title && data[title] && Object.keys(data[title]).some(key => !isNaN(key)) && (
                    ...
                    style={{
                        backgroundImage:isMobile ?"url('/space/MINFO.png')": "url('/space/INFO.png')",
